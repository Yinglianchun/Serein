...
> **自部署访问风险：** Serein 保留 HTTP（含公网 IP）访问及相关网页操作的兼容支持。公网明文 HTTP 不提供传输加密，登录凭据、密钥与记忆内容存在泄露风险，不建议长期这样传输私人记忆；公网访问推荐配置 HTTPS 或使用加密隧道。公开求助时请遮挡真实实例地址、账号密码与 Key，不要直接分享含密钥的连接说明。详见 [HTTP 兼容与访问风险](docs/interactive-install.md#http-兼容与访问风险)。
...
通过 HTTP 地址（包括局域网 IP 和公网 IP 加端口）打开网页时，也可以添加上游、手动添加模型和导入同名模型。曾遇到“添加上游”点击无反应的用户，升级后刷新页面即可使用。上述风险提示不改变这些功能，无需为使用这些按钮先配置 HTTPS。
